...
--user birth life pos lastPos status
data ClientInfo=ClientInfo Text (Maybe TimeSpec) (Maybe Int) Position (Maybe Position) Int deriving (Show,Eq)
instance ToJSON ClientInfo where
  toJSON (ClientInfo user birth life pos lastPos sta)=object ["user".=user,
                                                         "birth".=birth,
                                                         "life".=life, "pos".=pos,
                                                         "lastPos".=lastPos,
                                                         "status".=sta
...
clientToMsg (Client user conn birth life pos lastPos _ sta)
  =ClientMsg (ClientInfo user (Just birth) (Just life) pos (Just lastPos) sta)
...
--  WS.sendTextData sock $ encode (map getUser clients)
  if (userExists user clients) then do
    ...
    let life=600
    pos<-randomPos
    let newClient=(Client user sock birth life pos pos True 0)
...
--            liftM getClients (readState serState) >>= broadcast (T.concat [getUser client, " : ", msg])
            case msg of
              Just (ClientMsg (ClientInfo  _ _ _ pos _ sta))->do
                let newClient=client {getPos=pos,status=sta}
